...
**It cannot detect bare personal names or free-text content.** This is worth calling out explicitly because it is the most common blind spot. Paydirt detects values by shape (regex patterns for IPs, emails, tokens, SSNs, etc.) or by structured field name (JSON keys, CSV columns, Splunk fieldsummary fields targeted with `@json` rules). It cannot detect a value like `John Smith` in an unstructured `_raw` event, a customer name in a `comment` or `description` field, an internal project or customer code name, a person-named asset such as `JSMITH-LAPTOP`, branch or team names, or stack traces that echo user input. Two-capitalized-word patterns are not a viable detection signal because their false-positive rate on log data is catastrophic ("New York", "User Agent", "Last Modified", every product name). For structured cases the `@json` shortcut closes the gap; for unstructured narrative, reviewer attention is the only control. **Treat manual review of the scrubbed output as a residual control requirement, not an optional sanity check.** Compliance reviewers evaluating Paydirt for inclusion in a workflow should document this responsibility on the human reviewer in the relevant SOP, not assume the tool closes it.
